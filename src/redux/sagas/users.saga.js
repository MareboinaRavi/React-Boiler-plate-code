...
function* getAllUsers({ url }) {
  try {
    let users = yield call(axios.get, url);
...
function* getApi({ url }) {
  try {
    let users = yield call(axios.get, url);
    console.log(users.data, "from saga");
    yield put({ type: "GET_API_SUCCESS", data: users.data });
  } catch (error) {
    yield put({ type: "GET_API_FAILED", message: error.message });
  }
}
